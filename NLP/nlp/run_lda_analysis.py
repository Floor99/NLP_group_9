...
def main(app_name, series, nr_of_topics, nr_of_words_per_topic, start, limit, step):
    ...
    save_coherence_values(
        ...
        path= f"nlp/results/topic_vs_coherence_data/{app_name}.csv"
    )
    create_coherence_plot(
        nr_of_topics=topics,
        coherence_values=coherence_values,
        path= f"nlp/results/topics_vs_coherence_plot/{app_name}.png"
    )
    ...
    word_weight_path = f"nlp/results/topic_word_weights_data/{app_name}.csv"
    _ = save_lda_topics(
    ...
    vis = create_intertopic_distance_map(
        ...
        corpus=corpus,
        id2word=id2word,
        path=f"nlp/results/intertopic_distance_map/{app_name}.html"
    )
    return vis
